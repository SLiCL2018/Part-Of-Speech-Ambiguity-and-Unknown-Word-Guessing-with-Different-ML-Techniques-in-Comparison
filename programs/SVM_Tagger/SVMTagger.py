...
def penn_treebank():
    tagged_sentence = treebank.tagged_sents()
    ...
    # print(extract_features(tagged_sentence[0],1))
    ...
    null_tags = [(token, 'NULL') for token in textambig]
    input = [extract_features(null_tags, i) for i in range(len(null_tags))]
    X_test = vectoriser.transform(input)

    printlist = list(zip(textambig, clf.predict(X_test)))
    print("treebank ambig")
    print(printlist)

    # EVALUATION

    getridoffirstlist = textambigGoldStandard[0]
    y_true = [x[1] for x in getridoffirstlist]
    # print(y_true)

    y_pred = [x[1] for x in printlist]
    # print(y_pred)

    eval = sklearn.metrics.accuracy_score(y_true, y_pred, normalize=True, sample_weight=None)
    print("accuracy")
    print(eval)
    print("")
    gold = [str(tag) for sentence in textambigGoldStandard for token, tag in sentence]
    print(metrics.classification_report(gold, y_pred))
    print("")

    null_tags = [(token, 'NULL') for token in textnewwords]
    input = [extract_features(null_tags, i) for i in range(len(null_tags))]
    X_test = vectoriser.transform(input)

    printlist = list(zip(textnewwords, clf.predict(X_test)))
    print("")
    print("treebank newwords")
    print(printlist)

    # EVALUATION

    getridoffirstlist = textnewwordsGoldStandard[0]
    y_true = [x[1] for x in getridoffirstlist]
    # print(y_true)
...
def browncorpus():
    ...
    getridoffirstlist = textambigGoldStandardBrown[0]
    y_true = [x[1] for x in getridoffirstlist]
    # print(y_true)
    ...
    printlist = list(zip(textnewwords, clf.predict(X_test)))
    print("")
    print("brown newwords")
    print(printlist)

    # EVALUATION

    getridoffirstlist = textnewwordsGoldStandardBrown[0]
    y_true = [x[1] for x in getridoffirstlist]
    # print(y_true)
...
def connll():
    tagged_sentence = conll2000.tagged_sents()
...
def masc():
    tagged_sentence = (masc_tagged.tagged_sents()[0:1592])
