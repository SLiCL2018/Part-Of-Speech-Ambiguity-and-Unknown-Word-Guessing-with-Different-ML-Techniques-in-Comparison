...
def tagger1():
    ...
    def tagging1():
        tnt_tagger.tag(textambig)
    print("Time to tag ambig: ", timeit.timeit(tagging1, number=1))
    def tagging2():
        tnt_tagger.tag(textnewwords)
    print("Time to tag unknown: ", timeit.timeit(tagging2, number=1))
    print("")
    print('Evaluation TnTTagger, treebank, ambiguous words:')
    print("")
    print(tnt_tagger.tag(textambig))
    print("")
    print('accuracy: ', tnt_tagger.evaluate(textambigGoldStandard))
    gold = [str(tag) for sentence in textambigGoldStandard for token, tag in sentence]
    pred = [str(tag) for sentence in zip(tnt_tagger.tag(textambig)) for token, tag in sentence]
    print(metrics.classification_report(gold, pred))
    print("")
    print('Evaluation TnTTagger, treebank, unknown words:')
    print("")
    print(tnt_tagger.tag(textnewwords))
    print("")
    print('accuracy: ', tnt_tagger.evaluate(textnewwordsGoldStandard))
    gold = [str(tag) for sentence in textnewwordsGoldStandard for token, tag in sentence]
    pred = [str(tag) for sentence in zip(tnt_tagger.tag(textnewwords)) for token, tag in sentence]
    print(metrics.classification_report(gold, pred))
    print("")
...
def tagger2():
    ...
    tnt_tagger2 = tnt.TnT()
    tnt_tagger2.train(train2)
    def training():
        tnt_tagger2 = tnt.TnT()
        tnt_tagger2.train(train2)
    print("Time to train: ", timeit.timeit(training,number=1))
    def tagging1():
        tnt_tagger2.tag(textambig)
    print("Time to tag ambig: ", timeit.timeit(tagging1, number=1))
    def tagging2():
        tnt_tagger2.tag(textnewwords)
    print("Time to tag unknown: ", timeit.timeit(tagging2, number=1))
    print("")
    print('Evaluation TnTTagger, brown, ambiguous words:')
    print("")
    print(tnt_tagger2.tag(textambig))
    print("")
    print('accuracy: ', tnt_tagger2.evaluate(textambigGoldStandardBrown))
    gold = [str(tag) for sentence in textambigGoldStandardBrown for token, tag in sentence]
    pred = [str(tag) for sentence in zip(tnt_tagger2.tag(textambig)) for token, tag in sentence]
    print(metrics.classification_report(gold, pred))
    print("")
    print('Evaluation TnTTagger, brown, unknown words:')
    print("")
    print(tnt_tagger2.tag(textnewwords))
    print("")
    print('accuracy: ', tnt_tagger2.evaluate(textnewwordsGoldStandardBrown))
    gold = [str(tag) for sentence in textnewwordsGoldStandardBrown for token, tag in sentence]
    pred = [str(tag) for sentence in zip(tnt_tagger2.tag(textnewwords)) for token, tag in sentence]
    print(metrics.classification_report(gold, pred))
    print("")
...
def tagger3():
    ...
    tnt_tagger3 = tnt.TnT()
    tnt_tagger3.train(train3)
    def training():
        tnt_tagger3 = tnt.TnT()
        tnt_tagger3.train(train3)
    print("Time to train: ", timeit.timeit(training,number=1))
    def tagging1():
        tnt_tagger3.tag(textambig)
    print("Time to tag ambig: ", timeit.timeit(tagging1, number=1))
    def tagging2():
        tnt_tagger3.tag(textnewwords)
    print("Time to tag unknown: ", timeit.timeit(tagging2, number=1))
    print("")
    print('Evaluation TnTTagger, conll, ambiguous words:')
    print("")
    print(tnt_tagger3.tag(textambig))
    print("")
    print('accuracy: ', tnt_tagger3.evaluate(textambigGoldStandard))
    gold = [str(tag) for sentence in textambigGoldStandard for token, tag in sentence]
    pred = [str(tag) for sentence in zip(tnt_tagger3.tag(textambig)) for token, tag in sentence]
    print(metrics.classification_report(gold, pred))
    print("")
    print('Evaluation TnTTagger, conll, unknown words:')
    print("")
    print(tnt_tagger3.tag(textnewwords))
    print("")
    print('accuracy: ', tnt_tagger3.evaluate(textnewwordsGoldStandard))
    gold = [str(tag) for sentence in textnewwordsGoldStandard for token, tag in sentence]
    pred = [str(tag) for sentence in zip(tnt_tagger3.tag(textnewwords)) for token, tag in sentence]
    print(metrics.classification_report(gold, pred))
    print("")
...
def tagger4():
    ...
    tnt_tagger4 = tnt.TnT()
    tnt_tagger4.train(train4)
    def training():
        tnt_tagger2 = tnt.TnT()
        tnt_tagger2.train(train4)
    print("Time to train: ", timeit.timeit(training,number=1))
    def tagging1():
        tnt_tagger4.tag(textambig)
    print("Time to tag ambig: ", timeit.timeit(tagging1, number=1))
    def tagging2():
        tnt_tagger4.tag(textnewwords)
    print("Time to tag unknown: ", timeit.timeit(tagging2, number=1))
    print("")
    print('Evaluation TnTTagger, masc, ambiguous words:')
    print("")
    print(tnt_tagger4.tag(textambig))
    print("")
    print('accuracy: ', tnt_tagger4.evaluate(textambigGoldStandard))
    gold = [str(tag) for sentence in textambigGoldStandard for token, tag in sentence]
    pred = [str(tag) for sentence in zip(tnt_tagger4.tag(textambig)) for token, tag in sentence]
    print(metrics.classification_report(gold, pred))
    print("")
    print('Evaluation TnTTagger, masc, unknown words:')
    print("")
    print(tnt_tagger4.tag(textnewwords))
    print("")
    print('accuracy: ', tnt_tagger4.evaluate(textnewwordsGoldStandard))
    gold = [str(tag) for sentence in textnewwordsGoldStandard for token, tag in sentence]
    pred = [str(tag) for sentence in zip(tnt_tagger4.tag(textnewwords)) for token, tag in sentence]
    print(metrics.classification_report(gold, pred))
    print("")
